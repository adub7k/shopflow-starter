...
app.get('/api/booking/info', (req,res) => {
  try {
    ...
    const depositSettings = s.deposit||{};
    ...
    res.json({
      ...
      blockedDates,
      deposit: {
        enabled: depositSettings.enabled && stripeConnected,
        amount:  depositSettings.amount||10,
        message: depositSettings.message||'A deposit is required to secure your appointment.',
      },
...
app.post('/api/booking/request', async (req,res) => {
  ...
  const appt = { id:apptId, customerName, customerPhone, customerEmail:customerEmail||'', barberId:barberId||null, barberName:barberName||null, serviceId:serviceId||null, service:serviceName||'Haircut', price:confirmedPrice, duration:confirmedDuration, date, time, status: req.body.status==='pending-deposit' ? 'pending-deposit' : 'confirmed', notes:notes||'', source:'booking-page', createdAt:new Date().toISOString() };
...
function getStripe() {
  ...
  if (!key) throw new Error('STRIPE_SECRET_KEY not set in environment variables');
  ...
  try {
    ...
    if (!accountId) {
      const account = await stripe.accounts.create({
        ...
        metadata: { shopName: s.shopName||'ShopFlow Client' }
      });
...
app.get('/api/stripe/connect/return', async (req,res) => {
  try {
    ...
    db.get('settings').assign({ stripe:{ connectAccountId:accountId, onboardingComplete:complete } }).write();
...
app.get('/api/stripe/connect/status', async (req,res) => {
  try {
    ...
    const account = await stripe.accounts.retrieve(accountId);
    const complete = account.charges_enabled && account.payouts_enabled;
    if (complete !== s.stripe?.onboardingComplete) {
      db.get('settings').assign({ stripe:{ connectAccountId:accountId, onboardingComplete:complete } }).write();
    }
    res.json({ connected:complete, accountId, email:account.email||'' });
  } catch(e) {
...
// Disconnect Stripe
app.post('/api/stripe/connect/disconnect', (req,res) => {
  db.get('settings').assign({ stripe:{ connectAccountId:'', onboardingComplete:false } }).write();
  res.json({ ok:true });
});

// ── Deposit payment ───────────────────────────────────────────────────────────
// Creates a PaymentIntent on the barber's connected account
app.post('/api/deposit/create', async (req,res) => {
  try {
    const { appointmentId, customerName, customerEmail } = req.body;
    const s = db.get('settings').value()||{};
    const deposit = s.deposit||{};
    const stripeInfo = s.stripe||{};

    if (!deposit.enabled) return res.json({ ok:false, error:'Deposits not enabled' });
    if (!stripeInfo.connectAccountId) return res.json({ ok:false, error:'Stripe not connected' });

    const stripe = getStripe();
    const amount = Math.round((deposit.amount||10) * 100); // cents

    const paymentIntent = await stripe.paymentIntents.create({
      amount,
      currency: 'usd',
      payment_method_types: ['card'],
      description: `Deposit for appointment at ${s.shopName||'the shop'}`,
      receipt_email: customerEmail||undefined,
      metadata: { appointmentId: appointmentId||'', customerName, shopName: s.shopName||'' },
    }, {
      stripeAccount: stripeInfo.connectAccountId,
    });

    res.json({ ok:true, clientSecret: paymentIntent.client_secret, amount: deposit.amount, accountId: stripeInfo.connectAccountId });
  } catch(e) {
...
app.post('/api/deposit/confirm', (req,res) => {
  ...
  const a = getById('appointments', appointmentId);
  if (!a) return res.status(404).json({ ok:false });
  a.depositPaid = true;
  a.depositAmount = amount;
  a.depositPaymentId = paymentIntentId;
  a.status = 'confirmed'; // upgrade from pending-deposit to confirmed
  upsert('appointments', a);
  res.json({ ok:true });
});

// Waive deposit for a specific appointment (barber can do this from CRM)
app.post('/api/appointments/:id/waive-deposit', (req,res) => {
  const a = getById('appointments', req.params.id);
  if (!a) return res.status(404).json({ ok:false });
  a.depositWaived = true;
  upsert('appointments', a);
  res.json({ ok:true });
});
